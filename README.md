...
Current AI tools operate contrary to many users' expectations. Users who understand best-practices expect to be in charge of their tools; AI tools are designed to remove the user from the equation so they are "useful". This amplifies an existing risk: AI tools run with the same access as you do on your own computer. Any file you can change or delete, they can change or delete, and will do so without telling you. These problems aren't bugs; they are designed features of the automated tools. There is no way to keep users from using these tools, even if project policies exist that disallow it.
